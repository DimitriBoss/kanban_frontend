...
import { Plus, ArrowRight, Loader2, Folder, Pencil, Trash2, AlertTriangle } from "lucide-react";
...
function SortableBoardCard({ board, onNavigate, onEdit, onDelete }) {
  ...
  return (
    <div
      ...
      {...listeners}
      onClick={onNavigate}
      className={`group glass-panel project-card-animate p-6 rounded-xl min-h-[170px] flex flex-col justify-between cursor-grab active:cursor-grabbing border border-white/5 hover:border-indigo-500/20 hover:shadow-lg hover:shadow-indigo-500/5 hover:-translate-y-1 transition-[border-color,box-shadow,background-color,opacity] duration-200 ${
...
function BoardModal({ isOpen, mode, boardId, initialTitle = "", initialDescription = "", conflictStart = false, onClose, onSuccess, addToast }) {
  ...
  const handleFormSubmit = async () => {
    ...
    try {
      const response = mode === "CREATE"
        ...
        : await apiService.updateBoard(boardId, title.trim(), description.trim());
  ...
  const handleForceDuplicate = async () => {
    ...
    try {
      const response = mode === "CREATE"
        ...
        : await apiService.updateBoard(boardId, title.trim(), description.trim(), true);
  ...
  const handleConfirmRename = async () => {
    ...
    try {
      const response = mode === "CREATE"
        ...
        : await apiService.updateBoard(boardId, newTitle.trim(), description.trim());

      if (response?.status === "EXIST") {
        // Le nouveau nom est encore en conflit
        addToast(`Le nom "${newTitle.trim()}" est aussi déjà pris. Choisissez un autre nom.`, "error");
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/80 backdrop-blur-sm px-4">
      ...
      >
        {/* Halo d'arrière-plan */}
        <div className={`absolute top-0 right-0 w-[200px] h-[200px] rounded-full blur-[50px] pointer-events-none ${
          ...
          <div>
            <h3 className="text-lg font-bold text-white tracking-tight">
              {step === "form"
                ...
                : step === "conflict" ? "Projet existant"
                : "Renommer le projet"}
            </h3>
            {isConflictStep && (
              <p className="text-xs text-slate-500 mt-0.5">
                {step === "conflict" ? "Conflit de nom détecté" : "Saisir un nouveau nom"}
              </p>
            )}
          </div>
        </div>
        ...
        {step === "conflict" && (
          <div className="relative z-10">
            <p className="text-sm text-slate-300 leading-relaxed mb-6">
              Un projet nommé <span className="text-amber-400 font-semibold">&quot;{title}&quot;</span> existe déjà.
              Souhaitez-vous le renommer ou créer un doublon ?
            </p>
            <div className="flex items-center justify-end gap-3">
              ...
              <button
                onClick={() => { setNewTitle(title); setStep("rename"); }}
              ...
              >
                {isSubmitting
                  ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /><span>Création...</span></>
        ...
        {step === "rename" && (
          <div className="relative z-10">
            ...
            <div className="mb-5">
              <label className="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Nouveau nom</label>
              <input
                ref={renameInputRef}
                type="text"
                value={newTitle}
            ...
            <div className="flex items-center justify-end gap-3">
              ...
              <button
                ...
                disabled={!newTitle.trim() || newTitle.trim() === title || isSubmitting}
...
export default function Dashboard() {
  ...
  const [confirmModal, setConfirmModal] = useState({ isOpen: false, boardId: null });
  ...
  const closeBoardModal = () => setBoardModal({ isOpen: false, mode: "CREATE", boardId: null, initialTitle: "", initialDescription: "", conflictStart: false });
  ...
  useEffect(() => {
    const fetchBoards = async () => {
      try {
        ...
        const savedOrderStr = localStorage.getItem(`boards_order_${user?.id || "default"}`);
        if (savedOrderStr) {
    ...
    try {
      ...
      if (response && response.status === "EXIST") {
        ...
        setBoards((prevBoards) => {
          ...
          localStorage.setItem(`boards_order_${user?.id || "default"}`, JSON.stringify(ids));
          return updated;
        });
        setNewBoardTitle("");
        setNewBoardDescription("");
        addToast(`Le projet "${board.title}" a été créé avec succès !`, "success");
      }
    } catch (err) {
      console.error(err);
      addToast(err.message || "Erreur lors de la création du tableau. Veuillez réessayer.", "error");
    } finally {
      setIsCreating(false);
    }
  };
  ...
  const handleBoardSaved = (board) => {
    if (boardModal.mode === "CREATE") {
      ...
      setBoards((prev) =>
        prev.map((b) => (b._id || b.id) === (board._id || board.id) ? board : b)
      );
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="glass-panel p-6 rounded-xl flex flex-col justify-between border-dashed border-white/10 hover:border-indigo-500/30 transition-all duration-300">
              <div>
                ...
                <h3 className="text-lg font-bold text-white mb-3">Créer un tableau</h3>
              </div>
